...
class AICityLazySpatialDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        try:
            ...
            depth_image_tensor = process_depth(
                depth_image_basename, 
                self.data_args, 
                self.depth_image_folder
            )
        except Exception as e:
            print(f"Error loading or processing depth image {depth_image_basename} for sample {sources_dict['id']}: {e}")
        ...
        processed_source_conversations = preprocess_multimodal(
            [current_conversations], # Needs to be a list of conversation lists
            self.data_args
        ) 
