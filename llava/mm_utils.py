...
def process_masks(sources, data_args, image_info=None):
    # prepare processor
    processor = data_args.image_processor
    mask_processer = copy.deepcopy(processor)
    ...
    if modality == "rle":
        ...
        for rle in rles:
            ...
            m = m.astype(np.uint8)
            if data_args.image_aspect_ratio == "resize":
                m = cv2.resize(m, (crop_size["width"], crop_size["height"]), interpolation=cv2.INTER_NEAREST)
            if data_args.image_aspect_ratio == "pad":
                m = pad_to_square(m)
            masks.append(m)

    elif modality == "segmentation":
        polygens = sources[0]["segmentation"]
        image_info = sources[0]["image_info"]
        masks = []
        for i in range(len(polygens)):
            rle = cocomask.frPyObjects(polygens[i], image_info["height"], image_info["width"])
            m = cocomask.decode(rle)
        ...
        bboxes = sources[0]["bbox"]
        assert image_info is not None or "image_info" in sources[0]
        ...
        for bbox in bboxes:
            ...
            if data_args.image_aspect_ratio == "resize":
                zero_mask = cv2.resize(
                    zero_mask, (crop_size["width"], crop_size["height"]), interpolation=cv2.INTER_NEAREST
                )
            if data_args.image_aspect_ratio == "pad":
...
def process_depth(depth_file, data_args, depth_folder):
    processor = data_args.image_processor
...
def process_regions(masks, image_processor, data_args):
    ...
    # mask tensor
    masks_pt = torch.vstack(masks_pt).float()  # (n, h, w)
    return masks_pt
